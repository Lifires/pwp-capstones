...
def get_average_sentence_length(text):
    ...
    remove_question = text.replace('?','.')
    remove_exclamation = remove_question.replace('!', '.')
    sentences_in_text = remove_exclamation.split('.')
    
    number_of_sentences = len(sentences_in_text)-1
    
    word_count = []
    for sentence in sentences_in_text:
        words_in_sentence = sentence.split()
        word_count.append(len(words_in_sentence))
    
    words = 0
    for count in word_count:
        words += count

    average = words/number_of_sentences
    return average


def prepare_text(text):
    make_lower = text.lower()
    split_list = make_lower.split()
    punc_list = [".",";",":","!","?","/","\\",",","&",")","(","\""]
    for punc in punc_list:
        for word in split_list:
            split_list=[word.replace(punc,'') for word in split_list]
    return split_list
...
def ngram_creator(text_list):
    ...
    for word in range(len(text_list)-1):
        ngram_list.append((text_list[word]+ " " + text_list[word+1]))
    return ngram_list
...
class TextSample:
    def __init__(self, text, author):
        ...
        self.create_ngram = ngram_creator(self.prepare_text)
        self.ngram_frequency = build_frequency_table(self.create_ngram)
    def __repr__(self):
        return "{author} has an average sentence length of {sentence_length}.".format(author=self.author, sentence_length=self.average_sentence_length)
...
contestants = ["Myrtle Beech", "Lily Trebuchet", "Gregg T. Fishy"]
...
test_score = find_text_similarity(murderer_sample, murderer_sample)
scores = [myrtle_score, lily_score, gregg_score]
results = dict(zip(contestants, scores))

for contestant, score in results.items():
    print(contestant + " has a similarity score of " + str(score) + ".")

print("The person that killed Jay Stacksby is most likely to be {murderer}.".format(murderer = max(results, key=results.get)))
